...
- アプリの表示名・インストール名・短縮名は **「ぽこもこ」** とする（2026-09-08ユーザー採用）。ブラウザー、初回画面、印刷物のブランド表記も同名に統一する。既存の探索モード内のキャラクター「ポッコ」と、その物語・ゲーム名は改名しない。
- アプリアイコンは、紫の背景にクリーム／青の顔、水玉の丸耳、黄色い縫い目を持つパッチワークのくまを使う。正方形の元画像からfavicon・Apple touch・PWA各サイズを出力し、目鼻・口をmaskableの中心安全領域に保つ。名前とアイコン以外の画面構成・学習・島の挙動・保存キーは変更しない。
