...
### Lifeの孤立配置と歩行規則の切替（worldデータ版15）

[48](48_island_life_spec.md)の孤立予告と自由配置は、独立Life DBのworld版15と `placementCutover` で開始する。store/index版と学習DBのschemaは変えない。切替には規則名 `placement-access-v1`、所有者、時刻、旧action数と原文、SHA-256を保存し、既存の経済・巡回・施設・関係選択の切替情報を保持する。

旧actionは同時刻でも切替前、新しいactionは切替後として再現する。切替直前の所持・成長・通貨・住人の訪問を保持し、新規則の遡及適用で過去のひかりを変えない。以後の状態は `placementVersion: 1` を持ち、細分化した歩行経路は小数座標を含むが、所有物の配置は整数マスのままとする。移動時間・関係距離は経路の点数ではなく、従来の1マスを単位にした実距離で求める。

最新owner行の読取・移行・意図ID/内容/競合確認・配置再検証・保存を既存の一トランザクションとPWA保護内で行う。失敗時は元の行を保持する。版15で切替記録がない、旧履歴が変わった、検証hashが不正な場合は保存を拒否する。旧アプリへデータ版を下げない。復旧用リリースも版15を読めるコードを保持する。
