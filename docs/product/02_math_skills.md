...
- 整数・小数・分数・帯分数・商/余りは、正規の答えの各欄に対応する桁マスを最初から表示する。1桁でも複数桁でも最後の数字で正誤を問わず即時採点する。各欄が埋まると次の未入力欄へ移る。小数点は固定表示し、数字だけで入力できる。Enterや入力停止待ちは通常回答に不要。
...
- 数字の値は埋めない。マス数だけを導出し、入力途中の正誤判定・タイマー採点は行わない。Backspaceは固定小数点を飛ばして数字を1つ消す。自動移動後の空欄では直前に入力した欄へ戻して消す。欄を任意順に埋めた場合は末尾から先頭へ巡回し、未入力欄へ移る。自動移動直後の `/` は空の欄を飛ばさない。矢印・欄の選択・消去は次の数字入力より先に反映し、入力中の訂正を維持する。既存フィールド長ではなく表示マスと同じ長さで完了を判定する。
