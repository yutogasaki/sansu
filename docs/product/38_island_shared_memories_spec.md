...
- 幅480px以下・高さ640px以下のスマートフォンでは、写真ありの一覧でも「しゃしんを とる」と下部ナビを重ねない。先頭サムネイルは一覧で見分けられる範囲で小さくし、撮影入口を44px以上で初期表示内に保つ。幅600px以上・高さ430px以下では写真一覧をスクロール領域、撮影操作を独立列に分け、下部ナビに隠れない位置に保つ。1枚表示では写真を大きく見られる既存の役割を維持する。
