...
> 状態: gameplayの **MVP-0/1**、run保存の **MVP-2a**、Study共通planner / SRS接続の **MVP-2b**、version付きactive checkpointによるrun再開は **MVP-2c**、3 / 3 / 2問segment予約は **MVP-2d**、rapid-loop適格性と全source解放guardは **MVP-2e** を実装済み。1ラン8行動・15ノードを3問単位の連問区間として遊ぶ。ユーザー実機で `classic-v1` の旧マキモドンから後続の別rendererへ切り替わるmixed-lineage FAILを確認したため、production defaultはcontainmentとして `snap-root-v1` へ変更する。cold-openは `dig-pop-carry-bloom-v3`、通常Q4〜Q8は `firefly-stumble-bloom-painted-v5` を使う。旧候補の採点は継承せず、実配信targetのexact-build監査と無文字5人テストが終わるまでformal visual承認はHOLDとする。保存済みactive runのopening IDは途中で変えず、新規runだけ現行defaultを使う。本格グリッド、道具、鉱脈連鎖、未確定素材の損失は将来案として区別する。

## 1. コンセプト

本書は旧探索モードの仕様として維持する。[22_shared_subject_build_and_play_spec.md](22_shared_subject_build_and_play_spec.md) の遊園地へ固定8問・energy・帰還・旧checkpointを流用しない。
...
- `snap-root-v1` の現行containment visual candidate `dig-pop-carry-bloom-v3` は、スコップを持つ葉帽子のポッコ、土へ半分埋まった大きな根生物を、後続調査と同じ横camera・シアン・黄土・葉緑・コーラルの色面へ置き、`ready → dig-one → dig-two → popped` の3問で完結する。ポッコは根生物の身体や葉を引かず、中央の同じ土だけを掘る。正解ごとに一つの土塊が飛び、同じ根生物が持ち上がり、両足が見える。3問目は根生物が全身でぽんと抜け、ポッコが安全に尻もちをつき、柔らかい土塊が葉帽子へ載る。payoff variantは持たず、問題値や演算から見た目を選ばない。旧v2の合格点を継承せず、残る非補償ゲートが通るまでformal production採用はHOLDとする
- 1・2問目で生成されるopening途中の発見は保存してもtoastを出さない。問題と身体変化を同時に読ませ、3問目も説明modalや追加CTAを挟まず次の意味ある道選択へ進める
- production defaultは `snap-root-v1` とする。`classic-v1` はQ1〜Q3の旧マキモドンから後続の別rendererへ切り替わる既知の実機FAILであり、rollback先に使わない。旧編み根版Snap Rootは50 / 100のREJECT、一本葉を引くBloom版と水やり版はHOLDかつ非採用、`dig-pop-painted-v1` はsource art 50 / 60のHOLD、`dig-pop-painted-v2` は画風混在を残した旧runtime候補とする。現行cold-openは `dig-pop-carry-bloom-v3`、後続は `firefly-stumble-bloom-painted-v5`、`root-pull-v1` / `root-pull-v2` は比較用presentation、既存の専用遭遇 `root-tangle` は別契約とする。このdefault変更をformal Gate B通過とみなさず、delivery IDとvisual candidate IDを別々に記録する
...
- root-tangle縦切りでは同じ最終特徴IDへroot固有の観察「しずくの道が ひらいた！」を付け、commit済みattempt key、node、encounter、直近discoveryが一致した場合だけ観察へ進む
