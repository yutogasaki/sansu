...
Q7     前提を使う専用遭遇 → 大発見1件（blocking peak）
...
- Q4〜6で現行主調査の3手掛かりを順に開き、Q7の登録済み専用遭遇だけが最終特徴を開く。正答数や希少度だけで大発見にしない
...
- 固定10問fixtureでは、操作中断を `Q3 route / Q7 big discovery / Q8 return / replay` の4件へ固定し、2回目のrunのQ9〜10へ前runのdialog、world reaction、調査進捗を残さない
...
- 3つの通常手掛かりがそろったrunでは、保存済みroot回答と同じnode / encounter / attemptに結び付く解決だけが、最終特徴「ほたる花の ひかり道」と観察画を開く
...
- 最初の縦切りは、1ラン内の通常発見で開く3つの手掛かり特徴と、意味づけされた専用遭遇で開く1つの大発見を持つ
- 最終特徴は、3つの前提手掛かりと、catalogで指定した遭遇の保存済み解決がそろったときだけ開く。4件目の通常発見、正答数、希少度だけでは大発見にしない
...
- 1ラン 8行動、15ノード
- 2択中心、1層だけ3択、終盤は合流
- 深度1〜3は価値ゲート対象のcold-open、深度4〜6はほたる花の3手掛かり、深度7は既存root-tangleの大発見とする
