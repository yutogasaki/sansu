...
- 通常Q4〜Q7直前が `firefly-dew-path-painted-v3 / firefly-flower-side-v3` の同一横舞台で、一輪の花・一人のポッコ・四滴・一本の溝を保持すること。中立Q7だけが `firefly-q7-dew-path-v3` の `light-path` へ進み、三滴を道へ残して最後の一滴を葉帽子へ落とすこと
- 専用葉橋が `light-bridge-dew-path-v3 / light-bridge-side-v3`、専用root-tangleが `root-tangle-dew-path-v3 / root-tangle-side-v3` で通常調査と同じ横舞台・色面・役者・四滴を保持すること。旧縦長葉橋、旧密集root plate、全面発光や奥行きの強いplateがQ4→Q8 critical pathまたはPWA precacheへ残らないこと
- 2分岐と3分岐が `pokko-route-map-v3` の同一素材で、一本の溝から実際の選択肢数と同じ本数へ分かれること。旧密集林plateがroute thumbnail、大絵、基地予告へ表示されないこと
- 中立Q7の帰還図鑑は `firefly-field-book-painted-v3` の最終plateを使い、観察provenance付きroot-tangleはsource scene、camera key、candidate IDを帰還図鑑まで保持すること
