...
delivery / feature-flag ID `snap-root-v1` のlocal validationへ載せる現行visual candidateは `dig-pop-carry-bloom-v3` とする。`ready → dig-one → dig-two → popped` で、ポッコが根生物の身体や葉ではなく中央の同じ土を掘り、一つの土塊が飛ぶ、対象の両足が見える、全身でぽんと抜ける、ポッコが尻もちをついて柔らかい土塊が葉帽子へ載る、という一つの因果を作る。後続Q4〜Q8と同じ横camera・役者・大色面を使い、mobile / tabletは同じ4枚の1280×720 sourceを共有する。旧portrait / tablet専用v2画像をpublicとPWA precacheへ残さない。v3は旧v2のGate A採点、速度、同一build判定を継承せず、この候補を含むactual targetのcritical-path contact sheet、Gate B、Gate Cを改めて通すまでproduction判定をHOLDとする。
...
- 通常Q4〜Q8が `firefly-carry-bloom-painted-v4 / firefly-flower-side-v4` の同一横舞台で、一輪の花・一人のポッコ・四滴・一本の溝を保持すること。Q4から順に、葉帽子へ載せる、運ぶ、でこぼこで傾く、尻もちと開花、最後の一滴が葉帽子へ乗って帽子のふちが片目へかぶさる、という一つの因果を巻き戻さず進めること。中立Q7は `firefly-q7-carry-bloom-v4` の `light-path` を使うこと
...
- 中立Q7の帰還図鑑は `firefly-field-book-painted-v4` の最終plateを使い、観察provenance付きroot-tangleはsource scene、camera key、candidate IDを帰還図鑑まで保持すること
