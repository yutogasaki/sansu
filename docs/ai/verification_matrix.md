...
| `node tools/e2e-island-expression-audio-focused.mjs` | 初回1区間と必要な工作から三音の実資格/明示取得へ進み、既存audio phaseの実PCM・native時計・loop・無料音・hidden/退出/学習中停止を限定再検証する。固定20以降のmanifest・明示QA overlay・新規出力が必須で、初回予約と全tableの比較を各往復へ継承する。長い訪問/全取得/写真の経路を反復しない。実音の未計測prefix、実スピーカーと参加者の未検証は保持。環境変数と範囲は `--plan` を参照 |
...
| `node tools/e2e-island-camera.mjs` | phone/tabletで島のmouse/touch回転・ピンチ/ホイール/ボタン・縦scroll・取消・resize・DB不変・配置/学習復帰・reduced motion・WebGL復旧を検査。`SANSU_ISLAND_CAMERA_URL` と新しい `SANSU_ISLAND_CAMERA_OUTPUT` を指定。nativeプロフィールfixtureと実操作による作者の検査で、子どもの理解は未評価 |
