...
2026-09-09追補: [3仕事のphone復元診断09](../../design/audits/2026-09-09-island-shared-jobs/README.md)で実運搬・花並べ・照射→運搬記憶1件の再訪、本人配置3回、17store比較11回が限定PASS。5292/git8eの旧63回答データを明示復元した結果で、真正取得/両幅fullではない。[工作住民の固定39診断02](../../design/audits/2026-09-09-island-workshop-residents/README.md)は両幅でAの実水源操作・同選択保持・停止/取消・工房caller復帰・同予約保持を確認（旧7store復元、現17store比較22回）。運搬の手と照射結果の読みやすさ、工作の再入場待機の接地は視覚HOLD。接点の未撮影・実回答復帰・残る全組合せを別に保持し、工作の砂支持を仕様37に沿って修正中。
...
| R11 | 11. 仕草・交流 | 38の3仕事と40の[選択住民による実演](../../../src/components/island/three/optionalFurnitureController.ts)を実装。同一カップ受渡し、接眼、布への支持を扱い、基本交流を通貨で失わせない | 固定24の選定18家具利用は確認済み。旧shared-memories-06/shared-camera-03の3仕事はnot-run。後続の診断09は3仕事/運搬記憶再訪をphoneで限定確認したが、手の遮蔽と照射の視覚はHOLD。固定19の鳥/蝶の選定近景は作者による改善確認に到達したが、全3仕事/全住民ペア・配置での因果可読性は未合格。仕草単体の取得や同種内の新個体を、家具3品の実装や来訪近景の改善で充足扱いにしない | 採用・既実装/改善・検証中/P1 | 固定17家具06（`../../../output/island-experience/furniture-06/report.json`）の両幅で同じ住民の選定利用・本人による配置復旧がPASS。全仕事/全衣装の証拠へ広げず、物の出所→手→相手→返しと停止/学習復帰を実画像で継続確認 |
...
| E19 | 揃えて眺める / 住民の反応 | **既実装・実経路検証残**: 3仕事、3家具の接眼/布支持/同一cup、同じrigの衣装を接続。固定17の取得/本人復旧、固定24の有限18利用は別の限定PASS。3仕事→運搬記憶1件の再訪はphone復元診断09の限定PASS。手の遮蔽/照射結果はHOLDで、全ペア/全配置へ広げない。 | W03/W08/P1 |
...
1. 診断09で3仕事各1回→運搬記憶1件の再訪をphoneで限定確認した。次は実画像で見つけた手の遮蔽/照射の読みやすさを改善し、残る途中停止/同予約への実回答、他記憶・両幅fullを仕様38 M01〜M04/M12へ照合する。工作ではnav02の浮く待機を先に修正する。
