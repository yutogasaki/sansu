...
- 高優先の未所有課題がないか全ルートを継続棚卸しする。Explore F-30は[修正・5viewport実描画確認済み](../../design/audits/2026-09-20-whole-app-ux/README.md#f-30--wide-explore-prompt-and-instruction-overlap-the-answer-keypad-high-fixed-and-runtime-checked)。Study skipの被覆は[修正・実操作とsmoke確認済み](../../design/audits/2026-09-20-whole-app-ux/README.md#f-31--study-skip-action-was-covered-by-the-question-layer-on-phone-medium-fixed-and-runtime-checked)。Island shellの主要導線は`live-45`と現行候補の`live-50`/`live-54`/`live-55`/`live-56`でphone/tablet/desktop/短い横画面を確認したが、全状態網羅ではない。Battle setup/play/resultは`live-58`/`live-59`/`live-60`でphone/tablet/desktop/短い横画面を部分確認済み。保護者の空状態は1280×720・768×1024・390×844、履歴あり状態は390×844・768×1024、flag-offの苦手候補表示fixtureは`live-36`、現行Island親画面の復習候補表示は390×844・768×1024の`live-56`で部分確認した。Nature Townを含むflag-on構成と全ルート全状態網羅は未完了。
- `live-58`は「ほかのあそび」から2人ゲーム設定、`live-59`は協力Battleのsetup→play→時間切れ→replay→終了、`live-60`は綱引きsetup→play→勝利→result→replayを追加確認した。綱引きresult/replayは5 playable viewport、768×1024は回転案内で確認。全問題図の種類、実端末のスクロール感、Nature Townを含むflag-on構成、全ルート全状態網羅は引き続き未完了。
