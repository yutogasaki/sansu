...
- 次のゲーム面の候補は「置いた場所が住民のいつもの場所になる」。現状の種別差は主にしぐさで、家具への物理的な結果と自動訪問の選び方は共通。帰島/配置確定を契機に、住民ごとの好みから一度だけ既存の家具へ実経路で向かう案を優先する。保存した配置が次回も暮らしへ反映され、既存の住民追加でも同じ島の使い方が増える。常時巡回や新しい収集条件を増やす案ではなく、具体的な好み・優先・学習開始時の中断規則は次の実装前に28へ仕様化する。この候補は未実装・未観察であり、長期継続/関係成長の完成とは扱わない。
