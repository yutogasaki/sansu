...
|---|---|---|---|
| PWA cache/update drift | Users can stay on stale code after deploy, especially on mobile/PWA surfaces | [runbooks/pwa-release.md](runbooks/pwa-release.md), [runbooks/release-checklist.md](runbooks/release-checklist.md), release verification | Hosting, cache headers, update flow, or service worker behavior changes |
| Local storage and schema migration issues | Existing profiles can break or lose compatibility after data-shape changes | [runbooks/schema-migration.md](runbooks/schema-migration.md), [verification_matrix.md](verification_matrix.md), targeted tests | Storage keys, profile shape, migration logic, or persistence behavior changes |
