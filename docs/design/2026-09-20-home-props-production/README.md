...
[公開版の確認記録](public/report.json)。`https://sansu-seven.vercel.app` の実revision `288384543aba94ae217deaf4a0bf6835393fbb30` でphone/tabletの4点読込・候補ID・page errorなしを確認した。[phone近景](public/390-close.png) / [tablet近景](public/768-close.png)。新しい隔離ブラウザのローカルプロフィールで初回設定から確認し、既存利用者のデータには触れていない。後続の証跡保存コミットはdocsのみでアプリ入力は同じ。
