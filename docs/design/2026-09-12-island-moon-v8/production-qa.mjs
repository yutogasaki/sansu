...
try {
 for (const [name, viewport] of [['phone', { width: 390, height: 844 }], ['tablet', { width: 768, height: 1024 }]]) {
    ...
    try {
      ...
      assert.equal(await page.locator('[data-life-candidate]').getAttribute('data-life-candidate'), 'island-life-moon-garden-v8');
      ...
      report.scenarios.push({ name, pass: true, answers, earned, earnedCue: 6, earnedCueBox, residentFavorites, discoveryBounce: "unverified: transient hop absent from sampled DOM in runs b/c", residentReply: 'におい すき', rabbitHeadTilt: true, lightMeaning: true, lightStylePrice: 4,
