...
- 小数の固定点をまたぐBackspaceは1操作で数字1つを削除する。`12.3` から2回消して `1` に戻れる。
- 複数欄を任意順に入力すると、最後の欄から先頭の未入力欄へ巡回する。自動移動直後のBackspaceは、位置の隣接関係ではなく直前に入力した欄を戻す。
- 自動移動直後の `/` は未入力欄を飛ばさない。移動できない端の矢印や同じ欄への選択は、直前入力の情報を失わせない。
- Studyの欄選択・矢印・数字入力は同じイベント列でも同期した下書きを使う。他のcontenteditable欄の数字・長押しを学習の入力として処理しない。
- 先に回帰テストを追加して、小数の誤った再入力、欄の行き止まり、`/`による欄飛ばし、巡回後の削除先の誤りを再現してから修正した。
- 関連26ファイル436テスト合格、最後の矢印境界追加後のフォーム19テスト合格。実ブラウザ8ケース（Study/島×390/768幅×通常keyboard/同イベントburst）で正解と1件だけの保存を確認。burstは明示的な合成キー診断で、通常速度の測定ではない。初版ハーネスの「島の次問も同じ教材」仮定を外し、独立したnativeプロフィールfixtureで両操作を検査した。
- 詳細: ローカル `output/playwright/answer-cell-edges-boundaries/report.json`。既存の通常マス入力と保存失敗のテストも維持。
- `verify:core` の途中候補では3508件合格、今回触っていない家内3D mesh数テスト1件が失敗。main `38959dd` の統合でその既存失敗は解消し、該当5件を含む最終37テストとtypecheck/buildを再確認した。最終版全体のフルsuite再実行とは区別する。
