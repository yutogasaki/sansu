...
scenarios.splice(0, scenarios.length,
    { skill: 'add_1d_2', mathAnswerCount: 23, type: 'number', name: 'phone-make-ten', width: 390, height: 844, touch: true },
    { skill: 'add_1d_2', mathAnswerCount: 23, type: 'number', name: 'tablet-make-ten', width: 768, height: 1024 });
const report = { target: base, source, scenarios: [], captures: [], pass: false, humanN: 0,
    scope: 'Two supplementary make-ten hint/support scenarios using the native profile fixture with 23 prior answers, including 21 explicit independent correct answers. Native profile fixtures; actual planner reservations and trusted UI answers. Tests hint/draft/retry/reload/model/input blockade/supported next question and preserved mastery. Includes automatic single-digit/written answers. No reward or performance benchmark claim.' };
...
try {
    for (const scenario of scenarios) {
        ...
        try {
            ...
            // Explicit native-memory setup before any plan exists; no question/answer/receipt is injected.
            await page.evaluate(async id => {
                const open = indexedDB.open('SansuDatabase');
                const db = await new Promise((resolve, reject) => { open.onsuccess = () => resolve(open.result); open.onerror = () => reject(open.error); });
                const tx = db.transaction('memoryMath', 'readwrite'), store = tx.objectStore('memoryMath');
                const get = store.get([id, 'add_1d_2']);
                get.onsuccess = () => store.put({ ...get.result, independentCorrectAnswers: 21 });
            ...
            row.hintText = await page.locator('[data-support-kind=hint]').innerText(); assert(row.hintText.includes('あと') && row.hintText.includes('わけて'));
