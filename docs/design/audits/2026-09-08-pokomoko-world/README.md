...
- docs/lint/typecheck PASS。初回の全testは学習進行の1件が15秒のtimeout、残り2,231件PASS。関係する学習処理は変更していない。同時実行数を2に制限して全194ファイル・2,232件を再実行しPASS。assertやtimeoutは変更せず、元の失敗を[initial-timeout.log](initial-timeout.log)へ保存。負荷が疑われるが原因を断定しない。
