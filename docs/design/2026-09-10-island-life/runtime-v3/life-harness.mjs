...
const report = { target: base, flag: 'DEV + VITE_ISLAND_LIFE_PREVIEW=true', candidate: 'island-life-placement-v3', humanN: 0, clock: 'explicit DEV time advance; not real overnight evidence', scenarios: [] };
try {
 for (const [name, viewport, reducedMotion] of [['phone', { width: 390, height: 844 }, 'no-preference'], ['tablet-reduced', { width: 768, height: 1024 }, 'reduce']]) {
    ...
    const selectTab = async label => { await page.getByRole('group', { name: 'しまの ていれ' }).getByRole('button', { name: label, exact: true }).click(); };
    const worldCell = async (x, z, expanded = false) => {
        ...
        const aspect = box.width / box.height, width = expanded ? 9 : 6;
        const halfHeight = Math.max(3.8, (width + 1.8) / aspect / 2);
        const camera = new OrthographicCamera(-halfHeight * aspect, halfHeight * aspect, halfHeight, -halfHeight, .1, 100);
        camera.position.set(.3, 8, 10); camera.lookAt(0, .2, 0); camera.updateMatrixWorld();
        const point = new Vector3(x - (expanded ? 4 : 2.5), .047, z - 2).project(camera);
