...
- Whole-app UX/UI coherence：旧Exploreとの取り違え原因を整理し、通常devをIsland (`/#/island`)へ固定。core guardで起動設定・runtime identity・現行入口仕様を検査。旧Exploreは任意副モード、Nature Town (`5233`)は別preview。`live-55/56`の保護者表示、`live-58/59/60`の2人ゲーム導線・協力Battle・綱引き結果/再戦を確認。`live-60`でphone結果欄のはみ出しを修正し、6 viewport・48 capturesで再検証。全ルート/問題図状態、自然な復習判定、実読み上げ・参加者評価は継続 -> docs/tasks/active/2026-09-20-whole-app-ux-coherence.md
