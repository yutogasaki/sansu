...
- Whole-app UX/UI coherence：旧Exploreとの取り違え原因を整理し、通常devをIsland (`/#/island`)へ固定。core guardで起動設定・runtime identity・現行入口仕様を検査。旧Exploreは任意副モード、Nature Town (`5233`)は別preview。`live-55/56`で保護者の空/候補あり表示、`live-58`で現行IslandからOther Games→2人設定→戻りを5 viewport・92 capturesで確認。全ルート/状態の網羅、自然な復習判定、実読み上げ・参加者評価は継続 -> docs/tasks/active/2026-09-20-whole-app-ux-coherence.md
