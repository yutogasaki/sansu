...
export function IslandDistricts({ completedSets, value, disabled, onChange }: {
    completedSets: number; value: IslandDistrict; disabled: boolean; onChange: (value: IslandDistrict) => void;
}) {
    const places: { id: IslandDistrict; label: string; unlockAt: number }[] = [
        { id: 'all', label: 'しまぜんぶ', unlockAt: 0 }, { id: 'home', label: 'にわ', unlockAt: 0 },
        { id: 'east', label: 'ひがし', unlockAt: 2 }, { id: 'west', label: 'にし', unlockAt: 12 },
    ];
    return <div className="island-districts" role="group" aria-label="ながめる ばしょ">{places.filter(place => place.unlockAt <= completedSets).map(place =>
