...
describe('home optional action hierarchy', () => {
    it('keeps all existing routes and accessible names in the intended six-tile order', () => {
        ...
        expect(html.indexOf('<h3>あそぶ</h3>')).toBeLessThan(html.indexOf('<h3>ととのえる</h3>'));
        expect(html.indexOf('<h3>ととのえる</h3>')).toBeLessThan(html.indexOf('<h3>おもいで</h3>'));
    ...
    it('forwards each existing action once and gives a real gift its own arrival row', () => {
        ...
        expect(buttons(tree).map(button => button.props['data-home-action'])).toEqual([
            'rewards', 'play', 'guide', 'workshop', 'other-games', 'inventory', 'customization', 'experience', 'keepsakes', 'album', 'shared',
        ]);
        expect(html.indexOf('data-home-action="other-games"')).toBeLessThan(html.indexOf('<h3>ととのえる</h3>'));
    });
