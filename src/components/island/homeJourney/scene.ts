...
export function buildHomeJourney(state?: HomeJourneyState, options: { residentsOnly?: boolean; naturalOtter?: boolean } = {}) {
    ...
    const parcel = new T.Group(); parcel.name = 'parcel';
    if (!options.residentsOnly) { box(parcel, paint('#f5d273'), [0, 0, 0], [.36, .27, .27]);
