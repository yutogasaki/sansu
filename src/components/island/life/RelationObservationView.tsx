...
export default function RelationObservationView(props: Props) {
    ...
    useEffect(() => {
        ...
        const visibleObject = (object: T.Object3D, focus?: T.Vector3) => {
            ...
            return samples.some(sample => {
                const p = sample.clone().project(camera), rect = node.getBoundingClientRect();
                if (!node.contains(document.elementFromPoint(rect.left + (p.x + 1) / 2 * rect.width, rect.top + (1 - p.y) / 2 * rect.height))) return false;
