import FootstepReplay from './FootstepReplay';
...
export default function LifeSceneReplay({ original }: { original: DiscoveryScene }) {
    ...
    return <div data-life-replay-origin={original.originEventId ?? original.eventId}>
        ...
        {original.ruleId === 'X3' && bench ? <RelationObservationView state={frozen} frozen benchId={bench.id} residentId="otter" prepare={async () => undefined} presented={() => {}} readingReplay readingPrepare={prepare} readingPresented={(event, evidence) => { pending.current = { event, evidence }; void persist(); }} /> : (original.ruleId === 'X1' || original.ruleId === 'X2') && original.snapshot.scene.encounterTouch ? <RelationObservationView state={frozen} frozen gathering={{ ruleId: original.snapshot.scene.encounterTouch.normalRuleId, participantIds: original.snapshot.scene.encounterTouch.normalGroupIds }} prepare={async () => undefined} presented={() => {}} encounterReplay encounterPrepare={prepare} encounterPresented={(event, evidence) => { pending.current = { event, evidence }; void persist(); }} /> : original.ruleId === 'M1' ? <FootstepReplay state={frozen} profileId={original.profileId} prepare={prepare} presented={(event, evidence) => { pending.current = { event, evidence }; void persist(); }} /> : original.ruleId === 'M3' && bench ? <RelationObservationView state={frozen} benchId={bench.id} residentId={original.snapshot.scene.shadowTouch?.residentId} frozen prepare={async () => undefined} shadowPrepare={prepare} shadowPresented={(event, evidence) => { pending.current = { event, evidence }; void persist(); }} presented={() => {}} /> : original.ruleId === 'M4' && bench ? <WaterObservationView item={bench} conditionKey={original.semanticSignature} prepare={prepare} presented={(event, evidence) => { pending.current = { event, evidence }; void persist(); }} /> : original.ruleId === 'M2' && plant ? <>
