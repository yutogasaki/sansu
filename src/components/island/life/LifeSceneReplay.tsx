...
export default function LifeSceneReplay({ original }: { original: DiscoveryScene }) {
    ...
    return <div data-life-replay-origin={original.originEventId ?? original.eventId}>
        ...
        {original.ruleId === 'M1' ? <FootstepReplay state={frozen} profileId={original.profileId} prepare={prepare} presented={(event, evidence) => { pending.current = { event, evidence }; void persist(); }} /> : original.ruleId === 'M3' && bench ? <RelationObservationView state={frozen} benchId={bench.id} residentId={original.snapshot.scene.shadowTouch?.residentId} frozen prepare={async () => undefined} shadowPrepare={prepare} shadowPresented={(event, evidence) => { pending.current = { event, evidence }; void persist(); }} presented={() => {}} /> : original.ruleId === 'M4' && bench ? <WaterObservationView item={bench} conditionKey={original.semanticSignature} prepare={prepare} presented={(event, evidence) => { pending.current = { event, evidence }; void persist(); }} /> : original.ruleId === 'M2' && plant ? <>
