...
export function makeFacilityMotion(materials: IslandMaterials, bodies: T.Group[], heads: T.Group[], enabled: boolean, illustrated = false) {
    ...
    return (visible: LifeState, now: number, reduced: boolean) => {
        const result = new Map<string, { kind: 'library' | 'garden-hut'; action: 'reading' | 'tool-care' | 'carrying' }>();
        props.forEach((pair, index) => {
            ...
            const trip = resident.facilityTrip, carrying = trip?.phase === 'carry';
            if (!visit || now >= visit.end || !item || (!carrying && (!isFacility(item.kind) || activityPhase(visible, resident, now) !== item.kind))) return;
            const kind = carrying ? trip.kind : item.kind;
            const walking = carrying && activityPhase(visible, resident, now) === 'walking';
            const prop = kind === 'library' ? pair.book : pair.tools; prop.visible = true;
            prop.rotation.z = reduced ? 0 : Math.sin((now - visit.start) / (kind === 'library' ? 2200 : 800)) * .045;
            ...
            result.set(resident.id,{kind: kind as 'library' | 'garden-hut',action: walking ? 'carrying' : kind === 'library' ? 'reading' : 'tool-care'});
        });
        return result;
    };
}
