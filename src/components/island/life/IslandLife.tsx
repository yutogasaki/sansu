...
export default function IslandLife({ controls, onHome, disabled, islandName }: {
    ...
    const state = useMemo(() => record ? { ...replayLife(record), ...(import.meta.env.DEV && import.meta.env.VITE_ISLAND_LIFE_PREVIEW === 'true' ? { landscapeVersion: 'groves-water-v1' as const } : {}), worldStyle: import.meta.env.DEV && import.meta.env.VITE_ISLAND_LIFE_PREVIEW === 'true' ? 'canopy-dots-c3-v1' as const : 'moon-garden-v1' as const } : undefined, [record]);
    useEffect(() => {
