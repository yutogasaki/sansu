...
import type { LifeCommand, LifeState } from '../../../domain/islandLife/model';
import { placementClearanceEnd } from '../../../domain/islandLife/placementClearance';
...
export function usePlacementClearance(controls: ReturnType<typeof useIslandLife>, disabled: boolean) {
    ...
    const prepare = async (command: LifeCommand): Promise<number | undefined> => {
        ...
        const state = replayLife(record), kind = command.type === 'buy' ? command.kind : state.items.find(i => i.id === command.itemId)?.kind;
