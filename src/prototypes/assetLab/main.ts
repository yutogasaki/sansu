...
const candidate = garden ? 'island-asset-lab-garden-v1' : 'island-asset-lab-v1';
...
<aside><h2>テクスチャ</h2><div class="buttons"><button data-quality="2048">元の2K</button><button data-quality="1024">軽い1K</button>${garden ? '<button data-quality="runtime">配信用</button>' : ''}</div><p id="status" role="status" aria-live="polite"></p><button id="retry" hidden>読み込み直す</button><h2>見る場所</h2><div class="buttons"><button data-focus="all">島全体</button>${assets.map(a => `<button data-focus="${a.id}">${a.name}</button>`).join('')}</div><dl id="stats"></dl><p class="note">形・UV・法線は同一。家・住人・照明はゲームと共通です。配置・当たり判定は試験用です。</p><p class="note">読込時間はこのPC・現在の通信環境の参考値です。</p></aside></main><footer>${candidate} · dev-asset-lab · 本番未配信</footer>`;
...
let wanted: Quality = garden ? 'runtime' : '1024';
...
async function load(quality: Quality) {
    ...
    const results = await Promise.allSettled(assets.map(async a => {
        const path = quality === '1024' ? 'optimized/model-1024.glb' : 'final/model.glb';
        const gltf = await loader.loadAsync(`/assets/island-${a.id}-v1/${quality === 'runtime' ? 'runtime/near.glb' : path}`);
    ...
    if (failed) {
        ...
        status.textContent = `${quality === 'runtime' ? '配信用（圧縮テクスチャ）' : quality === '1024' ? '軽い1K' : '元の2K'}を表示中`;
        const idx = quality === 'runtime' ? 2 : quality === '1024' ? 1 : 0;
        const bytes = assets.reduce((sum, a) => sum + runtimeManifest[a.id as keyof typeof runtimeManifest].bytes[idx], 0);
        document.querySelector('#stats')!.innerHTML = `<dt>${assets.length}素材のGLB合計</dt><dd>${(bytes / 1e6).toFixed(2)} MB</dd><dt>三角形数</dt><dd>${triangles.toLocaleString()}</dd><dt>読込＋初回描画</dt><dd>${lab.loadMs} ms</dd>`;
    }
