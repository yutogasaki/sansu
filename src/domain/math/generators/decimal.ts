...
export const generators: Record<string, GeneratorFn> = {
    // Level 15: 小数の足し算
    "dec_add": (context) => {
        const a = randomInt(1, 99, context?.random) / 10; // 0.1 - 9.9
        const b = randomInt(1, 99, context?.random) / 10;
    ...
    // Level 15: 小数の引き算
    "dec_sub": (context) => {
        const a = randomInt(1, 99, context?.random) / 10;
        const b = randomInt(1, Math.round(a * 10), context?.random) / 10;
    ...
    "dec_div_int": (context) => {
        ...
        // q = 0.1 ~ 9.9
        const q = randomInt(1, 99, context?.random) / 10;
        const a = q * b;
