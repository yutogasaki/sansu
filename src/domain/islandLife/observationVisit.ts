...
export function observationVisit(state: LifeState, itemId: string) {
    ...
    const using = state.residents.find(resident => resident.visit && (resident.visit.itemId === itemId || resident.facilityTrip?.facilityId === itemId) && state.now < resident.visit.end);
    ...
    return { kind: 'ready' as const, ...free[0], duration: (free[0].path.length - 1) * LIFE_STEP_MS + (isFacility(item.kind) ? 15000 : 6000) };
}
