...
export function assertRelationCutover(record: LifeRecord) {
    ...
    if (!c) { if ((record.version === 13 || record.version === 14 || (record.version === 15 || (record.version === 16 || (record.version === 17 || (record.version === 18 || record.version === 19)))))) throw new Error('関係選択の切替記録が見つかりません。'); return; }
    if (![13, 14, 15, 16, 17, 18, 19].includes(record.version) || c.rules !== 'relation-selection-v1' || c.profileId !== record.profileId
...
export async function prepareRelationMigration(record: LifeRecord): Promise<LifeRecord> {
    if ((record.version === 13 || record.version === 14 || (record.version === 15 || (record.version === 16 || (record.version === 17 || (record.version === 18 || record.version === 19)))))) { await verifyRelationCutover(record); return record; }
