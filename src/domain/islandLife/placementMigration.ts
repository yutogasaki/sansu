...
export function assertPlacementCutover(record: LifeRecord) {
    ...
    if (!c) { if ((record.version === 15 || (record.version === 16 || record.version === 17))) throw new Error('配置の切替記録が見つかりません。'); return; }
    if (![15, 16, 17].includes(record.version) || c.rules !== 'placement-access-v1' || c.profileId !== record.profileId
...
export async function preparePlacementMigration(record: LifeRecord): Promise<LifeRecord> {
    if ((record.version === 15 || (record.version === 16 || record.version === 17))) { await verifyPlacementCutover(record); return record; }
