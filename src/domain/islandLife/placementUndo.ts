...
export function placementUndo(record: LifeRecord, actionId: string): LifeCommand | undefined {
    const spatial = record.actions.filter(a => ['buy', 'move', 'store', 'remove', 'expand'].includes(a.command.type));
    ...
    const previous = spatial.slice(0, -1).reverse().find(a => a.command.type === 'buy'
