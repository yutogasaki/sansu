...
export type ItemKind = 'flower' | 'bench' | 'swing' | 'lantern' | 'sapling' | 'water-bowl' | 'picnic-table' | 'pinwheel' | 'flower-arch';
...
export const CATALOG: Record<ItemKind, { label: string; price: number }> = {
    ...
    pinwheel: { label: 'かざぐるま', price: 12 }, 'flower-arch': { label: '花の アーチ', price: 12 },
};
...
export interface LifePurchaseReceipt {
    priceVersion: 'life-48-v1' | 'life-v3-plants-water-v1' | 'life-v3-picnic-v1' | 'life-v3-wind-arch-v1'; actualPaidDrops: number; quoteFingerprint: string;
...
export interface LifeRecord {
    profileId: string; version: 1 | 2 | 3 | 4 | 5 | 6 | 7 | 8; revision: number; createdAt: number; realAt: number; now: number;
...
export function readableLifeVersion(version: number) { return version === 1 || version === 2 || version === 3 || version === 4 || version === 5 || version === 6 || version === 7 || version === 8; }
