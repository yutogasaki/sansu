...
export function assertFacilityCutover(record: LifeRecord) {
    ...
    if (!c) { if (record.version === 11) throw new Error('運搬の切替記録が見つかりません。'); return; }
    if (record.version !== 11 || c.rules !== 'facility-trips-v1' || c.profileId !== record.profileId
...
export async function prepareFacilityMigration(record: LifeRecord): Promise<LifeRecord> {
    if (record.version === 11) { await verifyFacilityCutover(record); return record; }
