...
export function assertFacilityCutover(record: LifeRecord) {
    ...
    if (!c) { if ((record.version === 11 || record.version === 12)) throw new Error('運搬の切替記録が見つかりません。'); return; }
    if (![11, 12].includes(record.version) || c.rules !== 'facility-trips-v1' || c.profileId !== record.profileId
...
export async function prepareFacilityMigration(record: LifeRecord): Promise<LifeRecord> {
    if ((record.version === 11 || record.version === 12)) { await verifyFacilityCutover(record); return record; }
