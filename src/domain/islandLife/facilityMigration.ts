...
export function assertFacilityCutover(record: LifeRecord) {
    ...
    if (!c) { if ((record.version === 11 || record.version === 12 || (record.version === 13 || record.version === 14 || (record.version === 15 || (record.version === 16 || (record.version === 17 || (record.version === 18 || record.version === 19))))))) throw new Error('運搬の切替記録が見つかりません。'); return; }
    if (![11, 12, 13, 14, 15, 16, 17, 18, 19].includes(record.version) || c.rules !== 'facility-trips-v1' || c.profileId !== record.profileId
...
export async function prepareFacilityMigration(record: LifeRecord): Promise<LifeRecord> {
    if ((record.version === 11 || record.version === 12 || (record.version === 13 || record.version === 14 || (record.version === 15 || (record.version === 16 || (record.version === 17 || (record.version === 18 || record.version === 19))))))) { await verifyFacilityCutover(record); return record; }
