...
export function commandFingerprint(command: LifeCommand): string {
    switch (command.type) {
        ...
        case 'rotate': return JSON.stringify(['rotate', command.itemId, command.rotation]);
...
export function paidDrops(action: LifeAction): number {
    ...
    if ((isDecoration(action.command.kind) || isPlantsWater(action.command.kind) || action.command.kind === 'picnic-table' || isWindArch(action.command.kind) || action.command.kind === 'sandbox' || isFacility(action.command.kind)) && !receipt) throw new Error('新しい物の購入記録が見つかりません。');
