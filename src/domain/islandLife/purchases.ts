...
export const LEGACY_LIFE_PRICES: Readonly<Record<Exclude<ItemKind, 'sapling' | 'water-bowl' | 'picnic-table'>, number>> = Object.freeze({
...
function priceTerms(kind: ItemKind) {
    return kind === 'picnic-table' ? { version: 'life-v3-picnic-v1' as const, price: 8 } : isPlantsWater(kind) ? { version: 'life-v3-plants-water-v1' as const, price: PLANTS_WATER_PRICES[kind] }
...
export function paidDrops(action: LifeAction): number {
    ...
    if ((isPlantsWater(action.command.kind) || action.command.kind === 'picnic-table') && !receipt) throw new Error('新しい物の購入記録が見つかりません。');
