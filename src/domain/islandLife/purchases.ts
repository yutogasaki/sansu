...
export const LEGACY_LIFE_PRICES: Readonly<Record<Exclude<ItemKind, 'sapling' | 'water-bowl' | 'picnic-table' | 'pinwheel' | 'flower-arch'>, number>> = Object.freeze({
...
function priceTerms(kind: ItemKind) {
    return isWindArch(kind) ? { version: 'life-v3-wind-arch-v1' as const, price: 12 } : kind === 'picnic-table' ? { version: 'life-v3-picnic-v1' as const, price: 8 } : isPlantsWater(kind) ? { version: 'life-v3-plants-water-v1' as const, price: PLANTS_WATER_PRICES[kind] }
...
export function paidDrops(action: LifeAction): number {
    ...
    if ((isPlantsWater(action.command.kind) || action.command.kind === 'picnic-table' || isWindArch(action.command.kind)) && !receipt) throw new Error('新しい物の購入記録が見つかりません。');
