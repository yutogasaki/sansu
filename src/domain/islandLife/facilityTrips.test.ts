...
describe('facility transport migration',()=>{
    it('versions the new facility observation action without downgrading later purchases, land or refreshes',async()=>{
        ...
        try{await db.worlds.put(next);const refreshed=await updateLife('trip',[],undefined,100,db);expect(refreshed.version).toBe(16);expect(refreshed.actions).toEqual(next.actions);}finally{await db.delete();}
    });
    ...
    it('atomically rolls back and retries without changing old history or double revision increments',async()=>{
        ...
        try{
            ...
            expect(first.version).toBe(16);expect(first.revision).toBe(old.revision+1);
