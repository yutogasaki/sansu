...
export function assertCheckpointBoundary(record: LifeRecord) {
    ...
    if (!checkpoint) { if ((record.version === 3 || record.version === 4 || record.version === 5 || record.version === 6 || record.version === 7 || record.version === 8 || record.version === 9 || record.version === 10 || record.version === 11)) throw new Error('島の切替記録が見つかりません。'); return; }
    const known = new Set(checkpoint.projectedCreditIds);
    if (![3, 4, 5, 6, 7, 8, 9, 10, 11].includes(record.version) || record.profileId !== checkpoint.sourceRecord.profileId
...
export async function prepareEconomyMigration(record: LifeRecord, facts: readonly TerminalFact[], originalRecord: LifeRecord = record): Promise<LifeRecord> {
    if ((record.version === 3 || record.version === 4 || record.version === 5 || record.version === 6 || record.version === 7 || record.version === 8 || record.version === 9 || record.version === 10 || record.version === 11)) {
