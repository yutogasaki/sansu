...
export function assertCheckpointBoundary(record: LifeRecord) {
    ...
    if (!checkpoint) { if ((record.version === 3 || record.version === 4)) throw new Error('島の切替記録が見つかりません。'); return; }
    const known = new Set(checkpoint.projectedCreditIds);
    if (![3, 4].includes(record.version) || record.profileId !== checkpoint.sourceRecord.profileId
