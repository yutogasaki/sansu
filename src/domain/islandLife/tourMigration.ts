...
export function assertTourCutover(record: LifeRecord) {
    ...
    if (!cutover) { if (record.version === 4) throw new Error('巡回の切替記録が見つかりません。'); return; }
    if (record.version !== 4 || cutover.rules !== 'gp3-tours-v1' || cutover.profileId !== record.profileId
...
export async function prepareTourMigration(record: LifeRecord): Promise<LifeRecord> {
    if (record.version === 4) { await verifyTourCutover(record); return record; }
