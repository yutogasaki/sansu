...
export function assertCadenceCutover(record: LifeRecord) {
    ...
    if (!c) { if ((record.version === 16 || record.version === 17)) throw new Error('暮らしの切替記録が見つかりません。'); return; }
    if (![16, 17].includes(record.version) || c.rules !== 'resident-cadence-v1' || c.profileId !== record.profileId
...
export async function prepareCadenceMigration(record: LifeRecord): Promise<LifeRecord> {
    if ((record.version === 16 || record.version === 17)) { await verifyCadenceCutover(record); return record; }
