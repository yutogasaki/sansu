...
const candidate=sculpt?`canopy-sculpt-${sculpt}-study-v2`:shore?`canopy-shore-${shore}-study-v3`:`canopy-ground-${variant}-study-v1`;
...
const report={fault:failMesh?'explicit mesh network failure':failImage?'explicit texture network failure':'none',target:base,variant,shore,sculpt,candidate,flags:'DEV Island/Life preview; bark study=true; ground study='+variant+'; shore study='+(shore??'off')+'; sculpt study='+(sculpt??'off'),fixture:'Explicit same saved QA record, original resident assignments and logical time; realAt anchored at launch. No acquisition or human evidence.',startHash:await hash(),cases:[],pass:false};
