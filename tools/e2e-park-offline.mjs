...
try {
    ...
    await context.setOffline(true);await page.reload();await page.locator('[data-game-id]').waitFor();
    ...
    await fs.mkdir('output/playwright/park-art',{recursive:true});
    await fs.writeFile('output/playwright/park-art/offline-report.json',JSON.stringify(report,null,2));
    console.log(`PASS offline: ${loaded.length} sprites, replay and learning, real service worker`);
    await context.close();
