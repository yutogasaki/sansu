...
const browser=await chromium.launch();const report={target:base,version:await fetch(`${base}/version.json`).then(r=>r.json()),candidate:'nature-town-ground-a-v1',checks:[],pass:false};
try {
 for(const width of [390,768]){
  ...
  await page.goto(`${base}/#/nature-town`);await page.locator('[data-candidate="nature-town-ground-a-v1"]').waitFor();
  ...
  await context.setOffline(true);await page.reload();await page.locator('[data-candidate="nature-town-ground-a-v1"]').waitFor();
  await page.waitForFunction(()=>[...document.querySelectorAll('.town-resident-body img,.town-prop-art')].every(i=>i.complete&&i.naturalWidth===160));
