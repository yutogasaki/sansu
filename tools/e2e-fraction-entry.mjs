...
try {for(const width of [390,768])for(const route of ['island','study'])for(const skill of ['frac_sub_same','frac_mixed']) {
 ...
 const focus=async i=>tap(route==='island'?boxes.nth(i):boxes.nth(i).getByRole('button'));
 ...
 else {row.question=await p.locator('[data-study-index="0"]').getAttribute('data-question-text');const subtract=/[-−]/.test(row.question);const parts=row.question.replace(/\s*=\s*$/,'').split(/\s*[+−-]\s*/);const parse=x=>{const m=x.trim().match(/^(?:(\d+)\s+)?(\d+)\/(\d+)$/);assert(m,x);return [Number(m[1]||0)*Number(m[3])+Number(m[2]),Number(m[3])];};const [a,bb]=parts.map(parse);let nn=a[0]*bb[1]+(subtract?-1:1)*bb[0]*a[1],dd=a[1]*bb[1];const integer=Math.floor(nn/dd);if(count===3)nn%=dd;const gcd=(a,b)=>b?gcd(b,a%b):a;const g=gcd(nn,dd);values=(count===3?[String(integer)]:[]).concat(String(nn/g),String(dd/g));}
