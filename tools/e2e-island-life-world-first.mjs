    ...
    for (const [name, width, height] of [['phone', 390, 844], ['tablet', 768, 1024], ['small', 320, 568], ['landscape', 844, 390]]) {
        ...
        try {
            ...
            await page.evaluate(async () => {
                ...
                store.put({ ...rows[0], createdAt: start, now, realAt: now, activitiesV2At: start, activitiesV2After: 0, offsets: [{ at: start, offset: 0 }],
            ...
            await page.locator('.life-earned').waitFor({ state: 'hidden', timeout: 10000 });
