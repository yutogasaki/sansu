...
import { putCell } from './island-life-ui-helpers.mjs';
...
try {
    for (const [device, viewport] of [['phone', { width: 390, height: 844 }], ['tablet', { width: 768, height: 1024 }]]) {
        ...
        try {
            ...
            await context.setOffline(true); await page.reload(); await world(page); assert.equal(await bundle(page), bundles[1]); retained(before, await snapshot(page));
            ...
            assert.deepEqual(errors, []); report.cases.push({ device, viewport, cacheBefore, before, after, plan: partial.plan, navigations, errors, pass: true });
